...
import colored
from isaacgym.terrain_utils import SubTerrain, convert_heightfield_to_trimesh, pyramid_sloped_terrain, pyramid_stairs_terrain, random_uniform_terrain
import numpy as np
...
class LocalRunner(Runner):
    class _Simulator:
        ...
        def _create_envs(self) -> List[GymEnv]:
            ...
            heightfield = random_uniform_terrain(SubTerrain(width=512, length=512, vertical_scale=vertical_scale, horizontal_scale=horizontal_scale), min_height=-0.9, max_height=0.9, step=0.5, downsampled_scale=0.9).height_field_raw
            vertices, triangles = convert_heightfield_to_trimesh(heightfield, horizontal_scale=0.0125, vertical_scale=0.00025, slope_threshold=1.5)
